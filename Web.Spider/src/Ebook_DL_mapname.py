...
def ayanyzeData(data, url):
    try:
        ...
        result = list(result)
        link = result.pop(0)
        startIndex = link.rfind("/")
        downloadFileName = link[startIndex + 1: -1]
        
        title = titleRe.findall(data)
        title = list(title)
        fileName = title.pop(0)
        fileName = fileName[19: -8]
        
        printLog(downloadFileName + "\t" + fileName)
    except:
        printLog("error:" + url)
...
def downloadUrl(url):
    try:
        ...
        printLog("error: " + url)
ts = time()
links = [path % i for i in range(1, 9000)]
